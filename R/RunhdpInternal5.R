...
RunhdpInternal5 <-
  function(input.catalog,
    ...
    cat("setting global stir.closure\n")
    if (!exists("stir.closure", envir = .GlobalEnv)) {
      assign("stir.closure", xmake.s(), envir = .GlobalEnv)
    }
    cat("get env\n")
    ev <- environment(hdp::dp_activate)
    cat("unlock\n")
    unlockBinding("stirling", ev)
    cat("set stirling\n")
    ev$stirling <- .GlobalEnv$stir.closure
    cat("re-lock\n")
    lockBinding("stirling", ev)
    cat("set stirling done\n")
    ...
    # hdp::hdp_setdata generates the warning:
