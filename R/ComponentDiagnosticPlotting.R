...
ComponentDiagnosticPlotting <- function(
  retval,
  input.catalog,
  out.dir,
  verbose
  ...
  if (verbose) {
    message("Writing HDP diagnostics")
  }
  ...
  if (length(retval$signature) == 0) {
    ...
    grDevices::pdf(
      file.path(out.dir, "diagnostics.components.in.which.gibbs.samples.pdf")
    )
    graphics::par(mfrow = c(1, 1), mar = c(5, 4, 4, 2))
    hdpx::plot_component_posterior_samples(
      components = retval$signature,
